...
import { FaTrash, FaBan, FaTimes, FaPlus } from "react-icons/fa";
...
const NotaFiscalProduto: React.FC = () => {
  ...
  const [notasFiscaisProduto, setNotasFiscaisProduto] = useState<NotaFiscalProduto[]>([]);
  const [search, setSearch] = useState("");
  const [first, setFirst] = useState(0);
  const [rows, setRows] = useState(10);


  const filteredNotasFiscaisProduto = notasFiscaisProduto.filter((notaFiscalProduto) => {
    // Apenas ATIVO aparecem
    if (notaFiscalProduto.situacao !== 'Ativo') {
      return false;
    }

    // Lógica de busca
    return Object.values(notaFiscalProduto).some((value) =>
  ...
  const [notasFiscaisProdutoIdToDelete, setNotaFiscalProdutoIdToDelete] = useState<number | null>(null);
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [selectedNotaFiscalProduto, setSelectedNotaFiscalProduto] = useState<NotaFiscalProduto | null>(null);
  const [formValues, setFormValues] = useState<NotaFiscalProduto>({
  ...
  const handleSaveEdit = async (cod_natureza_operacao: any) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notasFiscaisProduto/edit/${cod_natureza_operacao}`,
        { ...formValues },
        {
          headers: {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        setLoading(false);
        clearInputs();
        fetchNotasFiscaisProduto();
        toast.success("Nota Fiscal de Produto salvo com sucesso!", {
          position: "top-right",
          autoClose: 3000,
        });
        setVisible(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        process.env.NEXT_PUBLIC_API_URL + "/api/notasFiscaisProduto/register",
        formValues,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const [rowData, setRowData] = useState<NotaFiscalProduto[]>([]);
  ...
  const handleSaveReturn = async (fecharTela: boolean) => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        setItemCreateReturnDisabled(false);
        setLoading(false);
        toast.error("Erro ao salvar Nota Fiscal de Produtos.", {
          position: "top-right",
          autoClose: 3000,
        });
      }
      ...
      setItemCreateReturnDisabled(false);
      setLoading(false);
      console.error("Erro ao salvar Nota Fiscal de Produtos:", error);
    }
  };
  ...
  const handleEdit = (notasFiscaisProduto: NotaFiscalProduto, visualizar: boolean) => {
    setVisualizar(visualizar);

    setFormValues(notasFiscaisProduto);
    setSelectedNotaFiscalProduto(notasFiscaisProduto);
    setIsEditing(true);
    setVisible(true);
  };
  ...
  const fetchNotasFiscaisProduto = async () => {
    setLoading(true);
    try {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_API_URL + "/api/notasFiscaisProduto",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      setRowData(response.data.notasFiscaisProduto);
      setIsDataLoaded(true);
      setNotasFiscaisProduto(response.data.notasFiscaisProduto);
      setLoading(false);
    } catch (error) {
      setLoading(false);
      console.error("Erro ao carregar Nota Fiscal de Produtos:", error);
    }
  };

  const openDialog = (id: number) => {
    setNotaFiscalProdutoIdToDelete(id);
    setModalDeleteVisible(true);
  };

  const closeDialog = () => {
    setModalDeleteVisible(false);
    setNotaFiscalProdutoIdToDelete(null);
  };

  const handleCancelar = async () => {
    if (notasFiscaisProdutoIdToDelete === null) return;

    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notasFiscaisProduto/cancel/${notasFiscaisProdutoIdToDelete}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status >= 200 && response.status < 300) {
        fetchNotasFiscaisProduto(); // Aqui é necessário chamar a função que irá atualizar a lista de naturezas de operacao
        setModalDeleteVisible(false);
      ...
      console.log("Erro ao excluir Nota Fiscal de Produto:", error);
      toast.error("Erro ao excluir Nota Fiscal de Produto. Tente novamente.", {
        position: "top-right",
        autoClose: 3000,
      });
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notasFiscaisProduto/${notasFiscaisProdutoIdToDelete}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      toast.success("Nota Fiscal de Produto removido com sucesso!", {
        position: "top-right",
        autoClose: 3000,
      });
      fetchNotasFiscaisProduto();
        ...
        <div className="flex justify-center">
          ...
          <div className="bg-grey pt-3 pl-1 pr-1 w-full h-full rounded-md">
            ...
            >
              ...
              <DataTable
                value={filteredNotasFiscaisProduto.slice(first, first + rows)}
                paginator={true}
                rows={rows}
...
export default NotaFiscalProduto;
