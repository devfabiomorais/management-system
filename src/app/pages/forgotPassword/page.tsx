...
export default function ForgotPassword() {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/users/sendCode", payload);
  ...
  const handleSubmitCode = async(e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/users/checkCode", payload);
  ...
  const handleSubmitNewPass = async(e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/api/users/changePassword/${idUser}`, payload);
      if (response.status === 200) {
        setLoading(false)
